...
/* Free a parsed tree: nodes, strings, and child arrays. The parser returns the
 * tree in separately calloc'd nodes (strings are individual allocations), so a
 * caller that is done with it must walk and free it rather than free one pointer. */
static void json_free(jval *v) {
    if (!v) return;
    if (v->t == J_STR && v->str) free(v->str);
