 ...
 *
 * DeepSeek-V4-Flash-0731 loads the released tokenizer.json through tok_load. Its
 * pre-tokenizer extensions and Unicode P/S tables are handled below without a Python
 * runtime dependency.
...
typedef struct {
    ...
    int deepseek;        /* 1 = DeepSeek-V4 family: number run, CJK/Kana run, then the
                          * P/S splitter (ASCII punct + letters, letter runs, symbol runs) */
...
static void tok_load(Tok *T, const char *path){
    ...
    /* merges: "left\0right" -> rank=i; the released format is a list of
     * "left right" strings (single ASCII space separator) */
    ...
    if(pt){
        ...
        if(ps&&ps->t==J_ARR) for(int i=0;i<ps->len;i++){
            ...
            if(rx&&rx->t==J_STR&&strstr(rx->str,"\\p{P}")) T->deepseek=1;
        }
    }
...
/* ---------- pre-tokenizer DeepSeek-V4 (official tokenizer.json) ----------
 * Sequence of three Split pretokenizers, then ByteLevel:
 *   1. \p{N}{1,3}                       digits, 1-3 codepoints
 *   2. [一-龥぀-ゟ゠-ヿ]+                 Han (4E00-9FA5) and Kana (3040-30FF)
 *   3. one Split whose regex has six alternatives, tried IN ORDER:
 ...
 *      e. \s+(?!\S)
 *      f. \s+
 * The P/S classes come from tok_unicode_ps.h; M from uni_M. The alternation is
 * leftmost-first like RE2, so rule a wins for "!hi", rule b for "héllo", etc. */
#define DS_CJK(c) (((c)>=0x4E00 && (c)<=0x9FA5) || ((c)>=0x3040 && (c)<=0x30FF))
#define DS_ASC_PUNCT(c) (((c)>=0x21 && (c)<=0x2F) || ((c)>=0x3A && (c)<=0x40) || \
                         ((c)>=0x5B && (c)<=0x60) || ((c)>=0x7B && (c)<=0x7E))
#define DS_ASC_LETTER(c) (((c)>='A' && (c)<='Z') || ((c)>='a' && (c)<='z'))
...
static void pretok_chunk_deepseek(Tok *T, const unsigned char *p, int a, int b, int *out, int *no, int max){
    ...
    while(i<n){
        ...
        if(is_N(c)){ int j=i,k=0; while(j<n && is_N(cp[j]) && k<3){ j++; k++; } i=j; bpe_piece(T,p,off[start],off[i],out,no,max); continue; }
        /* 2) Han + Kana run */
        if(DS_CJK(c)){ int j=i; while(j<n && DS_CJK(cp[j])) j++; i=j; bpe_piece(T,p,off[start],off[i],out,no,max); continue; }
        /* 3a) [ASCII punct][A-Za-z]+ */
        if(DS_ASC_PUNCT(c)){
            int j=i+1;
            while(j<n && DS_ASC_LETTER(cp[j])) j++;
            if(j>i+1){ i=j; bpe_piece(T,p,off[start],off[i],out,no,max); continue; }
        }
        /* 3b) [^\r\n\p{L}\p{P}\p{S}]?[\p{L}\p{M}]+ */
        {
            int j=i;
            int pref = !ISNL(c) && !is_L(c) && !is_PS(c);
            if(pref){ if(j+1<n && is_L(cp[j+1])) j++; else j=-1; }
            if(j>=0 && is_L(cp[j])){
                while(j<n && is_LM(cp[j])) j++;
                i=j; bpe_piece(T,p,off[start],off[i],out,no,max); continue;
            }
        }
        /* 3c) ' ?[\p{P}\p{S}]+[\r\n]*' */
        {
            int j=i;
            if(c==' ' && j+1<n && is_PS(cp[j+1])) j++;
            if(j<n && is_PS(cp[j])){
                while(j<n && is_PS(cp[j])) j++;
                while(j<n && ISNL(cp[j])) j++;
                i=j; bpe_piece(T,p,off[start],off[i],out,no,max); continue;
            }
        }
