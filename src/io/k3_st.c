/* k3_st.c - bounded safetensors reader adapted for DeepSeek-V4-Flash-0731.
 *
 ...
 *   for a large checkpoint header costs an allocation per node for no benefit. The
 *   scanner below walks the text once and writes straight into the index.
 *
 *   The obvious objection to a hand-written parser is that it can be subtly wrong. So
 *   it is not trusted: the synthetic safetensors tests exercise awkward ranks, dtypes
 *   and shard boundaries, while verify_dsv4_checkpoint.py checks every released tensor
 *   contract. The parser is also checked against an external implementation on real
 *   data, the same way every other part of this engine has been.
 */
...
int k3_st_elemsize(K3Dtype d)
{
    ...
    case K3_DT_I8:
    case K3_DT_F8_E4M3:
    case K3_DT_F8_E8M0: return 1;
    case K3_DT_BF16:
    case K3_DT_F16:     return 2;
    case K3_DT_F32:     return 4;
    case K3_DT_I64:     return 8;
    default:            return 0;
    }
}
...
static K3Dtype dtype_of(const char *s, size_t n)
{
    ...
    if (n == 2 && !memcmp(s, "I8", 2))      return K3_DT_I8;
    if (n == 3 && !memcmp(s, "I64", 3))     return K3_DT_I64;
    if (n == 4 && !memcmp(s, "BF16", 4))    return K3_DT_BF16;
    if (n == 3 && !memcmp(s, "F16", 3))     return K3_DT_F16;
    if (n == 3 && !memcmp(s, "F32", 3))     return K3_DT_F32;
    if (n == 7 && !memcmp(s, "F8_E4M3", 7)) return K3_DT_F8_E4M3;
    if (n == 7 && !memcmp(s, "F8_E8M0", 7)) return K3_DT_F8_E8M0;
                           ...
                           void *buf, int64_t bufcap, int64_t *payload_off)
{
    ...
    const int force_buffered = getenv("K3_ST_BUFFERED") != NULL;
    const int dfd = !force_buffered && s->dfd ? s->dfd[shard] : -1;
...
int64_t k3_st_read_f32(const K3St *s, const K3Tensor *t, float *out)
{
    ...
    while (done < n) {
        ...
        } else if (t->dtype == K3_DT_U8 || t->dtype == K3_DT_I8 ||
                   t->dtype == K3_DT_F8_E4M3 || t->dtype == K3_DT_F8_E8M0) {
            const unsigned char *p = (const unsigned char *)raw;
            for (int64_t i = 0; i < take; i++) o[i] = (float)p[i];
        } else if (t->dtype == K3_DT_I64) {
            const int64_t *p = (const int64_t *)raw;
            for (int64_t i = 0; i < take; i++) o[i] = (float)p[i];
